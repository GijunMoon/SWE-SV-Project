...
# 1. Hull 검출용 이미지 복제: 노이즈 최소화, 외곽선만 강조
img_hull = orig.copy()
gray_hull = cv2.cvtColor(img_hull, cv2.COLOR_BGR2GRAY)
gray_hull = cv2.GaussianBlur(gray_hull, (11, 11), 2)
edges_hull = cv2.Canny(gray_hull, 60, 130)
contours_hull, _ = cv2.findContours(edges_hull, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

hull_img = orig.copy()
if contours_hull:
    hull_contour = max(contours_hull, key=cv2.contourArea)
    cv2.drawContours(hull_img, [hull_contour], -1, (0, 0, 255), 5)
    x, y, ww, hh = cv2.boundingRect(hull_contour)
    cv2.putText(hull_img, 'Hull', (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2)
...
if left:
    ...
    cv2.circle(turret_img, (lmost[0], lmost[1]), lmost[2], (0,255,255), 3)
    cv2.putText(turret_img, 'Turret', (lmost[0], lmost[1]-10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,255), 2)
if right:
    rmost = max(right, key=lambda x: x[2])
    cv2.circle(turret_img, (rmost[0], rmost[1]), rmost[2], (0,255,255), 3)
    cv2.putText(turret_img, 'Turret', (rmost[0], rmost[1]-10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,255), 2)
...
for cnt in contours_feat:
    ...
    cx, cy = x + ww // 2, y + hh // 2
    aspect = ww / hh if hh != 0 else 0
    if 3 <= len(approx) <= 5 and 0.4 < aspect < 2:
        if w * 0.3 < cx < w * 0.7:
            cv2.drawContours(turret_img, [approx], -1, (0, 255, 0), 4)
            cv2.putText(turret_img, 'Bridge', (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)

cv2.imshow('Contours', turret_img)
cv2.imwrite('result_separate_processing.jpg', turret_img)
cv2.waitKey(0)
